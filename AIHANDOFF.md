...
> Version: 2.5.0 | Repo: Santimode/sierraphp | Package: santimode/sierraphp | Updated: 2026-09-01
> Brand: sierraPHP | Namespace: Sierra\ | PHP: ^8.2

This file is the single source of truth for AI agents.

### 1. Snapshot
- Repo: https://github.com/Santimode/sierraphp (public, active — MVP scaffold + full HTTP verbs & router completeness + Error Content Negotiation + Security Middleware + GitHub Actions CI done)
- Current state: Container, Router, Http (Request with method spoofing/inspection, Response, HttpException), Middleware (Stack, LogMiddleware, CorsMiddleware, SecurityHeadersMiddleware), Exceptions\Handler (with dual-mode content negotiation), helpers, and CI workflow all implemented and thoroughly tested. `composer test` passes (41 tests, 132 assertions).
...
```
Tests: full HTTP verbs (GET, POST, PUT, PATCH, DELETE, OPTIONS, HEAD, match, any) ✅, method spoofing ✅, group middleware & prefixes ✅, CorsMiddleware & SecurityHeadersMiddleware ✅, error content negotiation (JSON/HTML in debug & production) ✅, container auto-wire ✅, json response ✅, abort() helper ✅ (41 tests / 132 assertions)
...
### 5. File Map (v2.5.0 Scaffold)
- src/Application.php — creates container, router, runs dispatch; forwards all router verbs, wraps dispatch in try/catch, delegates to Exceptions\Handler on Throwable with Request context
...
- src/Exceptions/Handler.php — report(), renderDebugJson/Html, renderProductionJson/Html (content negotiated based on request Accept / format)
...
- tests/Feature/{ContainerTest,RequestTest,RouterTest,HttpExceptionTest,HandlerTest,HelperTest,MiddlewareTest}.php — Pest, 41 tests / 132 assertions
...
12. Swap `error_log()` in `Handler::report()` for a pluggable/structured logger (e.g. writing to `storage/logs/sierra.log` when the directory exists)

---
Changelog:
- 2.5.0: Added Error Content Negotiation (structured JSON vs pretty HTML in both debug and production modes), built-in security middleware (`CorsMiddleware`, `SecurityHeadersMiddleware`), and GitHub Actions CI workflow for PHP 8.2, 8.3, 8.4 (41 passing Pest tests / 132 assertions).
